...
public partial class DesmicologyOrderPage : ContentPage
{
    public DesmicologyOrderPage()
	{
		InitializeComponent();
    }
    ...
    private async Task<OrderWithDescriptionViewModel> CreateNewOrderAsync()
    {
        ...
        var currentOrder = new OrderWithDescriptionViewModel()
        {
            ...
            Description = $"Локализация грибка или плесени - {localizationEntry.Text} \n" +
            $"{text}"
        };
    ...
    private async void OrderButton_Clicked(object sender, EventArgs e)
    {
        ...
        {
            EmailManager.SendEmail("Новый заказ из приложения. Грибок и плесень", newOrderMesaageText);
