...
public partial class CommonCleaningOrderPage : ContentPage
{

    public CommonCleaningOrderPage()
	{
		InitializeComponent();
    }
    ...
    private async Task<OrderWithDescriptionViewModel> CreateNewOrderAsync()
    {
        ...
        var currentOrder = new OrderWithDescriptionViewModel()
        {
            ...
            Description = $"Что надо убрать - {cleaningSubjectEntry.Text} \n" +
            $"{text}"
        };
    ...
    private async void OrderButton_Clicked(object sender, EventArgs e)
    {
        ...
        {
            EmailManager.SendEmail("Новый заказ из приложения. Уборка", newOrderMesaageText);
